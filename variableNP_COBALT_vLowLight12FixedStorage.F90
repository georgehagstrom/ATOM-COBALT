...
namelist /generic_COBALT_nml/ do_14c, co2_calc, debug, do_nh3_atm_ocean_exchange, scheme_nitrif, &
  ...
  real, ALLOCATABLE, dimension(:,:)  :: &
     ...
     real, ALLOCATABLE, dimension(:,:,:)  :: &
          ...
          q_fe_2_n    , & 
          q_p_2_n     , & 
          q_p_2_n_opt , &    ! GIH 09/23/2020 Optimal P:N ratio, used to determine uptake
          silim       , & 
          q_si_2_n    , & 
          theta       , &
          EOpt       , &
        ...
        integer ::            &
          ...
          id_q_fe_2_n     = -1, &
          id_q_p_2_n      = -1, &
          id_q_p_2_n_opt  = -1, & ! GIH 
          id_silim        = -1, &
          id_q_si_2_n     = -1, & 
          id_theta        = -1, &
          id_f_p_100          = -1, & ! JYL ADD
          id_jprod_n_100  = -1, &
          id_jprod_n_new_100  = -1, &     
          id_sfc_f_p          = -1, &   ! JYL ADD 
          id_jprod_n_n2_100 = -1, &
          id_jzloss_n_100     = -1, &
          id_jaggloss_n_100   = -1, &
          id_jvirloss_n_100   = -1, &
  ...
  type bacteria
    ...
    real, ALLOCATABLE, dimension(:,:,:) :: &
          ...
          ldonlim,          & ! limitation due to organic substrate!q_p_2_n,          &
...
!    real                                  :: conversion 
    ...
    vardesc_temp = vardesc("irrlim_mem_Di","Diaz. Phyto. Light Limitation",'h','L','s','dimensionless','f')
    phyto(DIAZO)%id_irrlim_mem = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("irrlim_mem_Lg","Large Phyto. Light Limitation",'h','L','s','dimensionless','f')
    phyto(LARGE)%id_irrlim_mem = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("irrlim_mem_Sm","Small Phyto. Light Limitation",'h','L','s','dimensionless','f')
    phyto(SMALL)%id_irrlim_mem = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    ...
    vardesc_temp = vardesc("EOpt_Di","Diaz. Biosynthesis Investment",'h','L','s','gE/g C','f')
    phyto(DIAZO)%id_EOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("EOpt_Lg","Large Phyto. Biosynthesis Investment",'h','L','s','gE/g C','f')
    phyto(LARGE)%id_EOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("EOpt_Sm","Small Phyto. Biosynthesis Investment",'h','L','s','gE/gC','f')
    phyto(SMALL)%id_EOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)


    vardesc_temp = vardesc("LOpt_Di","Diaz. Light Investment",'h','L','s','gE/g C','f')
    phyto(DIAZO)%id_LOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("LOpt_Lg","Large Phyto. Light Investment",'h','L','s','gE/g C','f')
    phyto(LARGE)%id_LOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("LOpt_Sm","Small Phyto. Light Investment",'h','L','s','gE/gC','f')
    phyto(SMALL)%id_LOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)



    vardesc_temp = vardesc("LDiag_Di","Diaz. Light Investment",'h','L','s','None','f')
    phyto(DIAZO)%id_LDiag = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("LDiag_Lg","Large Phyto. Light Investment",'h','L','s','None','f')
    phyto(LARGE)%id_LDiag = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("LDiag_Sm","Small Phyto. Light Investment",'h','L','s','None','f')
    phyto(SMALL)%id_LDiag = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
! GIH Added q_p_2_n_opt
    vardesc_temp = vardesc("q_p_2_n_opt_Di","Optimal P:N ratio of Diaz. Phyto",'h','L','s','mol P/mol N','f')
    phyto(DIAZO)%id_q_p_2_n_opt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("q_p_2_n_opt_Lg","Optimal P:N ratio of Large Phyto",'h','L','s','mol P/mol N','f')
    phyto(LARGE)%id_q_p_2_n_opt = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("q_p_2_n_opt_Sm","Optimal P:N ratio of Small Phyto",'h','L','s','mol P/mol N','f')
    ...
    vardesc_temp = vardesc("jprod_nh4_Di","Diaz. phyto. NH4 Prod layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    phyto(DIAZO)%id_jprod_nh4 = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_nh4_Lg","Large phyto. NH4 Prodlayer integral",'h','L','s','mol NH4 m-2 s-1','f')
    phyto(LARGE)%id_jprod_nh4 = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_nh4_Sm","Small phyto. NH4 Prod layer integral", 'h','L','s','mol NH4 m-2 s-1','f')
    phyto(SMALL)%id_jprod_nh4 = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)


 
    vardesc_temp = vardesc("jprod_po4_Di","Diaz. phyto. PO4 Prod layer integral",'h','L','s','mol NH4 m-2 s-1','f')
    phyto(DIAZO)%id_jprod_po4 = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_po4_Lg","Large phyto. PO4 Prodlayer integral",'h','L','s','mol NH4 m-2 s-1','f')
    phyto(LARGE)%id_jprod_po4 = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("jprod_po4_Sm","Small phyto. PO4 Prod layer integral", 'h','L','s','mol NH4 m-2 s-1','f')
...
!    vardesc_temp = vardesc("jprod_pdi","Diazotroph Phosphorus production layer integral",'h','L','s','mol m-2 s-1','f')
!    phyto(DIAZO)%id_jprod_p = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
!         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

!    vardesc_temp = vardesc("jprod_psmp","Small phyto. Phosphorus production layer integral",'h','L','s','mol m-2 s-1','f')
!    phyto(SMALL)%id_jprod_p = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
!         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

!    vardesc_temp = vardesc("jprod_plgp","Large phyto. Phosphorus production layer integral",'h','L','s','mol m-2 s-1','f')
!    phyto(LARGE)%id_jprod_p = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
!         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
!    vardesc_temp = vardesc("q_p_2_n_Bact","P:N ratio of Bact",'h','L','s','mol P/mol N','f')
!    bact(1)%id_q_p_2_n = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
!         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
! CAS added a "plus_btm" version of jprod_nh4 to use for the remoc CMIP variable
    ...
    vardesc_temp = vardesc("expkPhotoT","Photosynthesis temperature limitation factor",'h','L','s','dimensionless','f')
    cobalt%id_expkPhotoT = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
! JYL ADD
 	vardesc_temp = vardesc("pphyto_tot","Total P: Di+Lg+Sm",'h','L','s','mol m-2 s-1','f')
 	cobalt%id_pphyto_tot = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
      init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    ...
    vardesc_temp = vardesc("sfc_day_length","Surface Day Length",'h','1','s','1/day','f')
    cobalt%id_sfc_day_length = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
! JYL ADD
 	     vardesc_temp = vardesc("sfc_psmp","Surface small phyto. phosphorus",'h','1','s','mol kg-1','f')
 	     phyto(SMALL)%id_sfc_f_p = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
 	          init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
! JYL ADD
 	     vardesc_temp = vardesc("sfc_plgp","Surface large phyto. phosphorus",'h','1','s','mol kg-1','f')
 	     phyto(LARGE)%id_sfc_f_p = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
 	          init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
! JYL ADD
 	     vardesc_temp = vardesc("sfc_pdi","Surface diazotroph phosphorus",'h','1','s','mol kg-1','f')
 	     phyto(DIAZO)%id_sfc_f_p = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
 	          init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    ...
    vardesc_temp = vardesc("sfc_q_p_2_n_opt_di","Opt Surface diazotroph phosphorus:nitrogen",'h','1','s','moles P (moles N)-1','f')
    phyto(DIAZO)%id_sfc_q_p_2_n_opt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_q_p_2_n_opt_smp","Opt Surface small phyto. phosphorus:nitrogen",'h','1','s','moles P (moles N)-1','f')
    phyto(SMALL)%id_sfc_q_p_2_n_opt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_q_p_2_n_opt_lgp","Opt Surface large phyto. phosphorus:nitrogen",'h','1','s','moles P (moles N)-1','f')
    phyto(LARGE)%id_sfc_q_p_2_n_opt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
    ...
    vardesc_temp = vardesc("sfc_q_p_2_n_di","Surface diazotroph phosphorus:nitrogen",'h','1','s','moles P (moles N)-1','f')
    phyto(DIAZO)%id_sfc_q_p_2_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_q_p_2_n_smp","Surface small phyto. phosphorus:nitrogen",'h','1','s','moles P (moles N)-1','f')
    phyto(SMALL)%id_sfc_q_p_2_n = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_q_p_2_n_lgp","Surface large phyto. phosphorus:nitrogen",'h','1','s','moles P (moles N)-1','f')
    ...
    vardesc_temp = vardesc("sfc_irrlim_mem_smp","Surface small phyto. light limitation",'h','1','s','dimensionsless','f')
    phyto(SMALL)%id_sfc_irrlim_mem = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_irrlim_mem_lgp","Surface large phyto. light limitation",'h','1','s','dimensionless','f')
    phyto(LARGE)%id_sfc_irrlim_mem = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_irrlim_mem_di","Surface diazotroph light limitation",'h','1','s','dimensionless','f')
    phyto(DIAZO)%id_sfc_irrlim_mem = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
   ...
   vardesc_temp = vardesc("sfc_LOpt_smp","Surface small phyto. LOpt",'h','1','s','gL/gC','f')
    phyto(SMALL)%id_sfc_LOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_LOpt_lgp","Surface large phyto. LOpt",'h','1','s','gL/gC','f')
    phyto(LARGE)%id_sfc_LOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_LOpt_di","Surface diazotroph LOpt",'h','1','s','gL/gC','f')
    phyto(DIAZO)%id_sfc_LOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)



   vardesc_temp = vardesc("sfc_LDiag_smp","Surface small phyto. LOpt",'h','1','s','None','f')
    phyto(SMALL)%id_sfc_LDiag = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_LDiag_lgp","Surface large phyto. LOpt",'h','1','s','None','f')
    phyto(LARGE)%id_sfc_LDiag = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_LDiag_di","Surface diazotroph LOpt",'h','1','s','None','f')
    phyto(DIAZO)%id_sfc_LDiag = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
   ...
   vardesc_temp = vardesc("sfc_EOpt_smp","Surface small phyto. EOpt",'h','1','s','gE/gC','f')
    phyto(SMALL)%id_sfc_EOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_EOpt_lgp","Surface large phyto. EOpt",'h','1','s','gE/gC','f')
    phyto(LARGE)%id_sfc_EOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_EOpt_di","Surface diazotroph EOpt",'h','1','s','gE/gC','f')
    phyto(DIAZO)%id_sfc_EOpt = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)



    vardesc_temp = vardesc("sfc_liebig_lim_smp","Surface small phyto. nutrient limitation",'h','1','s','dimensionsless','f')
    phyto(SMALL)%id_sfc_liebig_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_liebig_lim_lgp","Surface large phyto. nutrient limitation",'h','1','s','dimensionless','f')
    phyto(LARGE)%id_sfc_liebig_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

    vardesc_temp = vardesc("sfc_liebig_lim_di","Surface diazotroph nutrient limitation",'h','1','s','dimensionless','f')
    phyto(DIAZO)%id_sfc_liebig_lim = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
! CAS: Added diagnostic for diatom NPP in top 100m for CMIP
    ...
    vardesc_temp = vardesc("f_day_length","Cell Memory of Day Length ",'h','1','s',' s-1','f')
    cobalt%id_f_day_length = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)



    vardesc_temp = vardesc("f_day_flag","Cell Memory of Day Length ",'h','1','s',' none ','f')
    cobalt%id_f_day_flag = register_diag_field(package_name, vardesc_temp%name, axes(1:3),&
         init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
! JYL ADD
	vardesc_temp = vardesc("psmp_100","Small phytoplankton phosphorus biomass in upper 100m",'h','1','s','mol m-2','f')
     phyto(SMALL)%id_f_p_100 = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
	          init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)

      ! JYL ADD
    vardesc_temp = vardesc("plgp_100","Large phytoplankton phosphorus biomass in upper 100m",'h','1','s','mol m-2','f')
	     phyto(LARGE)%id_f_p_100 = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
	          init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
! JYL ADD
	vardesc_temp = vardesc("pdi_100","Diazotroph phosphorus biomass in upper 100m",'h','1','s','mol m-2','f')
	     phyto(DIAZO)%id_f_p_100 = register_diag_field(package_name, vardesc_temp%name, axes(1:2),&
	          init_time, vardesc_temp%longname,vardesc_temp%units, missing_value = missing_value1)
...
!    call g_tracer_add_param('o2_2_nfix', cobalt%o2_2_nfix, (118.0+3.0/(5.0+3.0)*(150.0-118.0))/16.0) ! mol O2 mol N-1
    ...
    call g_tracer_add_param('k_po4_Lg', phyto(LARGE)%k_po4,  1.0e-7)                  ! mol PO4 kg-1
    ...
    call g_tracer_add_param('fe_2_n_upt_fac', cobalt%fe_2_n_upt_fac, 30.0e-6)               ! mol Fe mol N-1
    !
    !-----------------------------------------------------------------------
    ! Phytoplankton light limitation/growth rate
    !-----------------------------------------------------------------------
    !
    call g_tracer_add_param('alpha_Di', phyto(DIAZO)%alpha,  0.8e-5 * 2.77e18 / 6.022e17/3.0)  ! g C g Chl-1 m2 J-1
    call g_tracer_add_param('alpha_Lg', phyto(LARGE)%alpha,  0.8e-5 * 2.77e18 / 6.022e17/3.0)  ! g C g Chl-1 m2 J-1 
    call g_tracer_add_param('alpha_Sm', phyto(SMALL)%alpha,  2.4e-5*2.77e18/6.022e17/3.0)      ! g C g Chl-1 m-2 J-1
    call g_tracer_add_param('kappa_eppley', cobalt%kappa_eppley, 0.063)                    ! deg C-1
    call g_tracer_add_param('kappa_photo', cobalt%kappa_photo, 0.063)                    ! deg C-1
    ...
    call g_tracer_add_param('radius_Di', phyto(DIAZO)%radius,1.0)                           ! mu m 
    call g_tracer_add_param('radius_Lg', phyto(LARGE)%radius,5.0)                     ! mu m
    call g_tracer_add_param('radius_Sm', phyto(SMALL)%radius,1.0)                     ! mu m 
    
    
    call g_tracer_add_param('kPhoto_Di', phyto(DIAZO)%kPhoto, 0.75*4e-6)                           ! mu m 
    call g_tracer_add_param('kPhoto_Lg', phyto(LARGE)%kPhoto, 0.75*4e-6)                     ! mu m
    call g_tracer_add_param('kPhoto_Sm', phyto(SMALL)%kPhoto, 0.75*4e-6)                     ! mu m 
    
   
    call g_tracer_add_param('PStorMax_Di', phyto(DIAZO)%PStorMax,0.01)                           ! gP/gCell 
    call g_tracer_add_param('PStorMax_Lg', phyto(LARGE)%PStorMax, .025)                     ! gP/gCell
    call g_tracer_add_param('PStorMax_Sm', phyto(SMALL)%PStorMax,.01)                 ! gP/gCell
          
    call g_tracer_add_param('PExtra_Di', phyto(DIAZO)%PExtra,0.002)                           ! gP/gCell
    call g_tracer_add_param('PExtra_Lg', phyto(LARGE)%PExtra,0.005)                     ! gP/gCell
    call g_tracer_add_param('PExtra_Sm', phyto(SMALL)%PExtra,0.0035)                     ! gP/gCell
 
    call g_tracer_add_param('k_po4_stor_max_Di', phyto(DIAZO)%k_po4_stor_max,  10.0e-7)                  ! mol PO4 kg-1
    call g_tracer_add_param('k_po4_stor_max_Lg', phyto(LARGE)%k_po4_stor_max,  10.0e-7)                  ! mol PO4 kg-1
    call g_tracer_add_param('k_po4_stor_max_Sm', phyto(SMALL)%k_po4_stor_max,  2.5e-8)                  ! mol PO4 kg-1
        
           
    call g_tracer_add_param('NS_Di', phyto(DIAZO)%NS,0.128)                           ! gN/gCell
    call g_tracer_add_param('NS_Lg', phyto(LARGE)%NS,0.128)                     ! gN/gCell
    call g_tracer_add_param('NS_Sm', phyto(SMALL)%NS,0.128)                     ! gN/gCell
 
 
    call g_tracer_add_param('cCrit_Di', phyto(DIAZO)%cCrit,  0.3034825870646766)
    call g_tracer_add_param('cCrit_Lg', phyto(LARGE)%cCrit,  0.3034825870646766  )
    call g_tracer_add_param('cCrit_Sm', phyto(SMALL)%cCrit,   0.3034825870646766)
    
    call g_tracer_add_param('a0_Di',phyto(DIAZO)%a0, 0.611510656625  ) 
    call g_tracer_add_param('a0_Lg',phyto(LARGE)%a0, 0.611510656625  )
    call g_tracer_add_param('a0_Sm',phyto(SMALL)%a0,  0.611510656625)
   
    call g_tracer_add_param('a1_Di',phyto(DIAZO)%a1, 0.22064845650928333 ) 
    call g_tracer_add_param('a1_Lg',phyto(LARGE)%a1, 0.22064845650928333  )
    call g_tracer_add_param('a1_Sm',phyto(SMALL)%a1, 0.22064845650928333)
  
    call g_tracer_add_param('a2_Di',phyto(DIAZO)%a2,-0.05382891709374325 )
    call g_tracer_add_param('a2_Lg',phyto(LARGE)%a2,-0.05382891709374325 )
    call g_tracer_add_param('a2_Sm',phyto(SMALL)%a2,  -0.05382891709374325)
    call g_tracer_add_param('a3_Di',phyto(DIAZO)%a3,0.01726321121940772 )
    call g_tracer_add_param('a3_Lg',phyto(LARGE)%a3,0.01726321121940772 )
    call g_tracer_add_param('a3_Sm',phyto(SMALL)%a3,   0.01726321121940772)
    call g_tracer_add_param('a4_Di',phyto(DIAZO)%a4,-0.0006751530906440923 )
    call g_tracer_add_param('a4_Lg',phyto(LARGE)%a4,-0.0006751530906440923 )
    call g_tracer_add_param('a4_Sm',phyto(SMALL)%a4, -0.006761491369354537)
    call g_tracer_add_param('ICrit_Di', phyto(DIAZO)%ICrit,  741.1877563729756  )
    call g_tracer_add_param('ICrit_Lg', phyto(LARGE)%ICrit, 741.1877563729756 )
    call g_tracer_add_param('ICrit_Sm', phyto(SMALL)%ICrit,  1482.3755127459513  )
    call g_tracer_add_param('aph_Di', phyto(DIAZO)%aph,1.972/3.0)
    call g_tracer_add_param('aph_Lg', phyto(LARGE)%aph,1.972/3.0)
    call g_tracer_add_param('aph_Sm', phyto(SMALL)%aph,1.972)
    call g_tracer_add_param('phiM_Di',phyto(DIAZO)%phiM, 1.0e-6)
    call g_tracer_add_param('phiM_Lg',phyto(LARGE)%phiM, 1.0e-6)
    call g_tracer_add_param('phiM_Sm',phyto(SMALL)%phiM, 1.0e-6)
    call g_tracer_add_param('k1_Di',phyto(DIAZO)%k1,0.74664)
    call g_tracer_add_param('k1_Lg',phyto(LARGE)%k1,0.74664)
    call g_tracer_add_param('k1_Sm',phyto(SMALL)%k1,0.74664)
    call g_tracer_add_param('k2_Di',phyto(DIAZO)%k2,1.7136)
    call g_tracer_add_param('k2_Lg',phyto(LARGE)%k2,1.7136)
    call g_tracer_add_param('k2_Sm',phyto(SMALL)%k2,1.7136)

    call g_tracer_add_param('LFac_Di',phyto(DIAZO)%LFac,1.2)   
    call g_tracer_add_param('LFac_Lg',phyto(LARGE)%LFac,1.2)
    call g_tracer_add_param('LFac_Sm',phyto(SMALL)%LFac,1.2)
 

    call g_tracer_add_param('zeta_Di',phyto(DIAZO)%zeta,0.25)   
    call g_tracer_add_param('zeta_Lg',phyto(LARGE)%zeta,0.05)
    call g_tracer_add_param('zeta_Sm',phyto(SMALL)%zeta,0.05)
 

    call g_tracer_add_param('thetaFac_Di',phyto(DIAZO)%thetaFac,0.031523104693140784*2.0)   
    call g_tracer_add_param('thetaFac_Lg',phyto(LARGE)%thetaFac,0.031523104693140784*2.0)
    call g_tracer_add_param('thetaFac_Sm',phyto(SMALL)%thetaFac,0.021015403128760525)
    ...
    !-----------------------------------------------------------------------
    !
    call g_tracer_add_param('p_2_n_static', cobalt%p_2_n_static, .false. )    !GIH
    ...
    !call g_tracer_add_param('p_2_n_static_Lg', phyto(LARGE)%p_2_n_static,1.0/12.0 )         ! mol P mol N-1    !GIH
    !call g_tracer_add_param('p_2_n_static_Sm', phyto(SMALL)%p_2_n_static,1.0/20.0 )         ! mol P mol N-1    !GIH
    ...
    call g_tracer_add_param('phi_ldop_vir',cobalt%lysis_phi_ldop, 0.75)    ! dimensionless
    call g_tracer_add_param('phi_srdop_vir',cobalt%lysis_phi_srdop, 0.05)  ! dimensionless
    ...
    call g_tracer_add_param('beta_fescav',cobalt%beta_fescav, 1.5e9/spery )                  ! sec-1 (mole ndet kg-1)-1
    ...
    call g_tracer_add_param('kfe_eq_lig_ll',cobalt%kfe_eq_lig_ll, 1.2e12)                    ! mol lig-1 kg
    call g_tracer_add_param('kfe_eq_lig_hl',cobalt%kfe_eq_lig_hl, 1.2e9)                     ! mol lig-1 kg
    ...
    call g_tracer_add_param('molarP',  cobalt%molarP, 31.0) !g/mol
    call g_tracer_add_param('molarN',  cobalt%molarN,14.0 )                     ! g/mol
    call g_tracer_add_param('NProt',   cobalt%NProt, 0.16)                         ! gN/gDry
    call g_tracer_add_param('alphaS', cobalt%alphaS, 0.25)                         ! g/gCell Dry
    call g_tracer_add_param('gammaS', cobalt%gammaS, 0.2)                         ! g/gCell Dry
    call g_tracer_add_param('PE', cobalt%PE, 0.05)                        ! GIH gP/g
    call g_tracer_add_param('day_cutoff', cobalt%day_cutoff, 1.0)                        ! GIH gP/g
...
! JYL ADD
 	     !
 	     !    PDi (assumed to be facultative N2-fixers, with a variable N:P ratio
 	     !
 	     call g_tracer_add(tracer_list,package_name,&
 	          name       = 'pdi',                 &
 	     ...
 	     !
 	     !    PLg (assumed to be a dynamic combination of diatoms and other
 	     !         eukaryotes all effectively greater than 5 um in diameter,
 	     !         and having a fixed C:N ratio)
 	     !
 	     call g_tracer_add(tracer_list,package_name,&
 	          ...
 	          prog       = .true.            )
 	     !
 	     !       PSm (Phosphorus in picoplankton and nanoplankton
 	     !            ~less than 5 um in diameter and having a fixed C:N:P ratio)
 	     !
 	     call g_tracer_add(tracer_list,package_name,&
 	          name       = 'psm',            &
  ...
  subroutine generic_COBALT_update_from_source(tracer_list,Temp,Salt,rho_dzt,dzt,hblt_depth,&
    ...
    ! Local Variables for Variable Stoichiometry Calculations
    real :: EMax, ENutLim, LNutLim, S_plank, AQuad, BQuad, CQuad, EMaxGrowth, LMaxGrowth, EOpt, LOpt, PStor, plim_stor, bresp_ref
    real :: LMin
    real :: ITrans, cPlank, aPhoto, PCM_s, kE
...
!
    ...
    call g_tracer_get_values(tracer_list,'pdi'    ,'field',phyto(DIAZO)%f_p(:,:,:) ,isd,jsd,ntau=tau,positive=.true.)  !JYL
    call g_tracer_get_values(tracer_list,'nlg'    ,'field',phyto(LARGE)%f_n(:,:,:) ,isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'plg'    ,'field',phyto(LARGE)%f_p(:,:,:) ,isd,jsd,ntau=tau,positive=.true.) !JYL 
    call g_tracer_get_values(tracer_list,'nsm' ,'field',phyto(SMALL)%f_n(:,:,:),isd,jsd,ntau=tau,positive=.true.)
    call g_tracer_get_values(tracer_list,'psm' ,'field',phyto(SMALL)%f_p(:,:,:),isd,jsd,ntau=tau,positive=.true.)    !JYL
    ...
    call g_tracer_get_values(tracer_list,'irrlim_mem_Di' ,'field',phyto(DIAZO)%irrlim_mem,isd,jsd,ntau=1)

    call g_tracer_get_values(tracer_list,'irrlim_mem_Lg' ,'field',phyto(LARGE)%irrlim_mem,isd,jsd,ntau=1)
    call g_tracer_get_values(tracer_list,'irrlim_mem_Sm' ,'field',phyto(SMALL)%irrlim_mem,isd,jsd,ntau=1)
    ...
    call g_tracer_get_values(tracer_list,'irr_mem','field',cobalt%f_irr_mem ,isd,jsd,ntau=1)
    call g_tracer_get_values(tracer_list,'f_day_flag','field',cobalt%f_day_flag ,isd,jsd,ntau=1)
    call g_tracer_get_values(tracer_list,'f_day_length','field',cobalt%f_day_length ,isd,jsd,ntau=1)
...
!
    ...
    !-----------------------------------------------------------------------------------
    !
    ! Calculate iron and phosphorus cell quota
    !
    do k = 1, nk  ; do j = jsc, jec ; do i = isc, iec
...
! JYL ADD
    phyto(n)%q_p_2_n(i,j,k) = max(0.0, phyto(n)%f_p(i,j,k)/ & ! may not need the max(0.0,p/n) in future
                  max(epsln,phyto(n)%f_n(i,j,k)))
    if ((phyto(n)%f_n(i,j,k) .LT. epsln ) .AND. (phyto(n)%f_p(i,j,k) .LT. epsln ))  then
            phyto(n)%q_p_2_n(i,j,k) = 1.0/22.0
    end if
                 !if (n .eq. 1) then
         !       phyto(n)%q_p_2_n(i,j,k) = phyto(n)%p_2_n_static
         !  else
         !       phyto(n)%q_p_2_n(i,j,k) = phyto(n)%f_p(i,j,k) / max(epsln,phyto(n)%f_n(i,j,k))
         !   end if
       enddo  !} n
       !
       ...
       do n= 1, NUM_PHYTO   !{
          if (scheme_no3_nh4_lim .eq. 1) then
             ...
             if (ISNAN(phyto(n)%no3lim(i,j,k))) then
                     phyto(n)%no3lim(i,j,k) = 0.0
             end if
             if (ISNAN(phyto(n)%nh4lim(i,j,k))) then
                     phyto(n)%nh4lim(i,j,k) = 0.0
             end if
       ...
       do n= 1, NUM_PHYTO   !{
          ...
          phyto(n)%def_fe(i,j,k) = phyto(n)%q_fe_2_n(i,j,k)**2.0 / (phyto(n)%k_fe_2_n**2.0 +  &
             ...
             if (ISNAN(phyto(n)%po4lim(i,j,k))) then
                     phyto(n)%po4lim(i,j,k) = 0.0
             end if
             if (ISNAN(phyto(n)%felim(i,j,k))) then
                     phyto(n)%felim(i,j,k) = 0.0
             end if
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{        
       ...
       do n= 2, NUM_PHYTO   !{
        ...
        if ( (phyto(n)%liebig_lim(i,j,k) > phyto(n)%no3lim(i,j,k) + phyto(n)%nh4lim(i,j,k))) then
                if (k .EQ. 1) then
                print *, "Problem with Liebig Lim", "Plankton Type", n, "NO3 Lim", phyto(n)%no3lim(i,j,k), &
                 " NH4 Lim ", phyto(n)%nh4lim(i,j,k),  " n_lim " , phyto(n)%no3lim(i,j,k)+phyto(n)%nh4lim(i,j,k), &
                " liebig lim ", phyto(n)%liebig_lim(i,j,k), " PO4 lim ", phyto(n)%po4lim(i,j,k), " Fe lim ", phyto(n)%def_fe(i,j,k), &
                " min factor ",   min(phyto(n)%no3lim(i,j,k)+phyto(n)%nh4lim(i,j,k),&
             phyto(n)%po4lim(i,j,k), phyto(n)%def_fe(i,j,k)), &
          "NO3", cobalt%f_no3(i,j,k), "NH4",cobalt%f_nh4(i,j,k), "PO4", cobalt%f_po4(i,j,k)
                                
                   call mpp_error(FATAL,"Limitation Mismatch")
                end if
        end if
    ...
    ! Update the day length and day flag variables if we are in the surface box:
    if ( k .eq. 1) then
                 cobalt%f_day_flag(i,j,1) = 0
                 if (cobalt%irr_mix(i,j,k) > cobalt%day_cutoff ) then
                        ! Set day flag to one 
                        cobalt%f_day_flag(i,j,1) = 1
                 endif
                 
                 ! Average in the time step to the day length variable
                 cobalt%f_day_length(i,j,1) = ( cobalt%f_day_length(i,j,1) + (cobalt%f_day_flag(i,j,1) - cobalt%f_day_length(i,j,1)) * &
                 min(1.0,cobalt%gamma_irr_mem*dt)   )   *grid_tmask(i,j,k)

    endif 
 


          cobalt%expkPhotoT(i,j,k) = exp(cobalt%kappa_photo * (Temp(i,j,k)-15.0)) 
          cobalt%expkT(i,j,k) = exp(cobalt%kappa_eppley * Temp(i,j,k))
          if (cobalt%f_day_flag(i,j,1) .gt. 0 ) then

                  cobalt%f_irr_mem(i,j,k) = (cobalt%f_irr_mem(i,j,k) + (cobalt%irr_mix(i,j,k) - &
                  cobalt%f_irr_mem(i,j,k)) * min(1.0,cobalt%gamma_irr_mem * dt / (cobalt%f_day_length(i,j,1)+EPSLN) ) * &
          grid_tmask(i,j,k) )
    ...
    !
    ! Phytoplankton growth rate/stoichiometry based on harmonized ATOM/COBALT model
    !
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
      ...
      cobalt%f_chl(i,j,k) = 0.0
       do n = 1, NUM_PHYTO   !{
            ...
            ! Here I am adding code which applies when the phytoplankton type is large or small (i.e. n = 2 or 3)
           
            ! GIH Temporary change in the loop to bypass this part of the code for debugging purposes
            ! GIH S_plank is the fraction of cell dry mass not part of the biosynthetic or photosynthetic apparatus
            S_plank = cobalt%alphaS/phyto(n)%radius+cobalt%gammaS
                
            ! GIH:
            ! The small and large phytoplankton both can store luxury P.
            ! Each type has a maximum capacity for storage, calibrating using our knowledge of their
            ! physiology. Small plankton store P in their phospholipids only, while large phytoplankton
            ! we assume to never substitute phospholipids and to have the capacity for a greater quantity of
            ! luxury storage. The half-saturation constant for P-storage is set 10 times higher than for
            ! P-limitation. plim_stor gets directly added to the structural P-quota to determine the overall
            ! stoichiometry 

 

                if (4.57*cobalt%f_irr_mem(i,j,k) > phyto(n)%ICrit*cobalt%expkPhotoT(i,j,k)) then


        cPlank =  phyto(n)%cCrit

        else
        ITrans = 2*4.57*cobalt%f_irr_mem(i,j,k)/(phyto(n)%ICrit*cobalt%expkPhotoT(i,j,k))-1

        cPlank = 1.0-sqrt(0.5*phyto(n)%a0 + phyto(n)%a1 * ITrans + phyto(n)%a2 * (2*ITrans**2-1) + phyto(n)%a3 * (4*ITrans**3-3*ITrans) + &
            phyto(n)%a4*(8*ITrans**4-8*ITrans**2+1))
        end if
            ...
            PCM_s = min(phyto(n)%k1*(1-cPlank),phyto(n)%k2*cPlank)/3600.0*cobalt%expkPhotoT(i,j,k)

            aPhoto = PCM_s*(1-exp(-phyto(n)%aph*phyto(n)%phiM*cPlank*4.57*cobalt%f_irr_mem(i,j,k)/PCM_s))





            plim_stor = cobalt%f_po4(i,j,k)/(phyto(n)%k_po4_stor_max+cobalt%f_po4(i,j,k))
            if (ISNAN(cobalt%f_po4(i,j,k))) then
                   ! 
                   ! GIH: Sometimes at initialization cobalt%f_po4(i,j,k) = NaN
                   plim_stor = 0.0
            end if
            ! Adjust respiration rate for temperature and refuge concentration
            bresp_ref = phyto(n)%bresp*cobalt%expkT(i,j,k)*phyto(n)%f_n(i,j,k)/(cobalt%refuge_conc + phyto(n)%f_n(i,j,k))
            kE = phyto(n)%P_C_max*cobalt%expkT(i,j,k)
            ! GIH: LMin is the level of investment implied by 0 investment in biosynthetic apparatus.
            ! IE this is the level which would lead to a growth rate of 0, with photosynthesis balancing respiratory
            ! losses. If this minimum investment in L is greater than the maximum total investment, ie if more L is needed
            ! than space available to avoid respiratory losses, we assume that the cell invests nothing in E and maxes out
            ! L to minimize the repiratory loss. Here we are taking respiratory losses independent of total investment (morally
            ! this is equivalent to basal respiration proportional to P_C_Max
           
    
            ENutLim = phyto(n)%liebig_lim(i,j,k)
            if (aPhoto .lt. 1.0e-18) then
                    LNutLim = 10.0
            else

                    LNutLim = (kE*ENutLim*(1+phyto(n)%zeta)+bresp_ref)/(cobalt%f_day_length(i,j,1)*aPhoto)*phyto(n)%LFac
            end if         

            


                    if ( LNutLim+ENutLim .gt. 1.0-S_plank ) then


             if (aPhoto .lt. 1.0e-18) then
                     EMaxGrowth = 0.0
                     LMaxGrowth = 1.0-S_plank-EMaxGrowth
            else



                        !EMaxGrowth = (1-S_plank)/(1 + (kE*(1+cobalt%zeta)+bresp_ref)/(cobalt%f_day_length(i,j,1)*aPhoto))
                        EMaxGrowth = ((1-S_plank)*cobalt%f_day_length(i,j,1)*aPhoto/phyto(n)%LFac-bresp_ref)/ &
                                ((kE*(1+phyto(n)%zeta))+(cobalt%f_day_length(i,j,1)*aPhoto/phyto(n)%LFac))

                        LMaxGrowth =  (kE*EMaxGrowth*(1+phyto(n)%zeta)+bresp_ref)/(cobalt%f_day_length(i,j,1)*aPhoto)*phyto(n)%LFac
             end if
                        if (EMaxGrowth .lt. 0) then
                                EMaxGrowth = 0
                                LMaxGrowth = 1-S_plank
                        end if


                            ! GIH I believe this case is impossible, but put this here just in case    
                        EOpt = EMaxGrowth
                        LOpt = LMaxGrowth
                        !nutlimFlag_lp = 0
                    else
                ...
                PStor = 0.3*phyto(n)%PStorMax ! Level of P Storage
                ! Note to self: Explain the edge cases and the carbon to chlorophyl formula!!!!!!!!!
                phyto(n)%theta(i,j,k) = cPlank *LOpt* phyto(n)%thetaFac/((S_plank+EOpt+LOpt)) ! Chlorophyl to Carbon


                !phyto(n)%q_p_2_n_opt(i,j,k) = 1.0/(    ((S_plank*phyto(n)%NS+cobalt%NProt*(EOpt+LOpt)) & 
                !        / (cobalt%PE*EOpt+phyto(n)%PExtra + PStor*(EOpt+LOpt+S_plank))*cobalt%molarP/cobalt%molarN))
                             
                if ( n .EQ. LARGE) then 
                phyto(n)%q_p_2_n_opt(i,j,k) = ((cobalt%PE*EOpt+phyto(n)%PExtra + &
                PStor*(EOpt+LOpt+S_plank))/(cobalt%molarP/cobalt%molarN))  &  
                /(S_plank*phyto(n)%NS+cobalt%NProt*(EOpt+LOpt)) 
                                        
                else                
                 phyto(n)%q_p_2_n_opt(i,j,k) = ((cobalt%PE*EOpt+phyto(n)%PExtra + &
                PStor*(S_plank))/(cobalt%molarP/cobalt%molarN))  &  
                /(S_plank*phyto(n)%NS+cobalt%NProt*(EOpt+LOpt)) 
end if
                ...
                phyto(n)%EOpt(i,j,k) = EOpt
                phyto(n)%LOpt(i,j,k) = LOpt

                ! GIH I made a choice here. irr_lim is needed but hard to interpret in context of colimitation. Also we treat
                ! respiratory losses differently, they come from photosynthesis and not the other terms
                phyto(n)%irrlim(i,j,k) = (PCM_s*LOpt*(1-exp(-phyto(n)%aph*phyto(n)%phiM*cPlank*4.57*cobalt%irr_inst(i,j,k)/PCM_s))-bresp_ref)  &
                /(1+phyto(n)%zeta)
                    

                phyto(n)%irrlim_mem(i,j,k) = phyto(n)%irrlim_mem(i,j,k)+(phyto(n)%irrlim(i,j,k)-phyto(n)%irrlim_mem(i,j,k))*min(1.0,cobalt%gamma_irr_mem*dt) *grid_tmask(i,j,k)

 
                phyto(n)%mu(i,j,k) = min(EOpt*kE,phyto(n)%irrlim_mem(i,j,k))
                phyto(n)%LDiag(i,j,k) = (EOpt*kE+epsln)/(phyto(n)%irrlim_mem(i,j,k)+epsln) 

                phyto(n)%jprod_n(i,j,k) = phyto(n)%mu(i,j,k)*phyto(n)%f_n(i,j,k)
                phyto(n)%mu_mix(i,j,k) =  phyto(n)%mu(i,j,k)
                ! Now check for sanity, we should have no NaN values in the growth rate :



        ! Test of small plankton
     !   if ( ( n .EQ. SMALL) .AND. (k .EQ. 1) .AND. (i .EQ. 20) .AND. (j .EQ. 9) ) then
     !           print *, " Testing Optimality Every Point Surface ", " NO3 ", cobalt%f_no3(i,j,k), " NH4 ", cobalt%f_nh4(i,j,k), " PO4 ", &
!                cobalt%f_po4(i,j,k), " Day Length ", cobalt%f_day_length(i,j,k), " irr mem ", cobalt%f_irr_mem(i,j,k), &
!                " irr inst ", cobalt%irr_inst(i,j,k), " Temp ", temp(i,j,k),  " aPhoto ", aPhoto, " PCM ",  PCM_s, &
!                " cPlank ", cPlank, " E Nutlim ", ENutLim, " LNutLim ", LNutLim, " EOpt ", EOpt, " LOpt ", LOpt, &
!                " N:P Opt ", 1.0/phyto(n)%q_p_2_n_opt(i,j,k), " Temp Factor ", cobalt%expkPhotoT(i,j,k), &
!               " kappa_photo ", cobalt%kappa_photo, " Temp Fact 2 ", exp(cobalt%kappa_photo*(Temp(i,j,k)-15.0)) ," i ", i, &
!                " j ", j, " no3lim ", phyto(n)%no3lim(i,j,k), " nh4lim ", phyto(n)%nh4lim(i,j,k), " po4lim ", &
!                phyto(n)%po4lim(i,j,k), " fe lim ", phyto(n)%felim(i,j,k), " liebig lim ", phyto(n)%liebig_lim(i,j,k)
...
!        end if
                ...
                if ((ISNAN(phyto(n)%mu(i,j,k))) .OR. ( (phyto(n)%mu(i,j,k)+1.0 ) .EQ. phyto(n)%mu(i,j,k) ) )then
                    
                    print *, "Likely FATAL NaN error in phyto(n)%mu", "Plankton Type", n, "Irradiance", cobalt%f_irr_mem(i,j,k), &
                    "Temperature", Temp(i,j,k), "NO3", cobalt%f_no3(i,j,k), "NH4",cobalt%f_nh4(i,j,k), "PO4", cobalt%f_po4(i,j,k),&
                    "FeLim", phyto(n)%def_fe(i,j,k), "EOpt", EOpt, "LOpt", LOpt, " expkT ", ( cobalt%expkT(i,j,k)),  " 1-S_plank ",   &
                         ((1-S_plank)), " day length ", cobalt%f_day_length(i,j,1), " aPhoto ", aPhoto, " kE " ,   kE , & 
                        " resp cost - ",  bresp_ref, " zeta tax ",   cobalt%zeta
 
                    call mpp_error(FATAL,"NaN/Inf in phytoplankton growth rate")
                    
                    
                end if

                if (ISNAN(phyto(n)%q_p_2_n_opt(i,j,k))  .OR.( (phyto(n)%q_p_2_n_opt(i,j,k)+1.0)  .EQ. phyto(n)%q_p_2_n_opt(i,j,k) )) then
  
                    print *, "Likely FATAL NaN error in phyto(n)%q_p_2_n", "Plankton Type", n, "Irradiance", cobalt%f_irr_mem(i,j,k), &
                    "Temperature", Temp(i,j,k), "NO3", cobalt%f_no3(i,j,k), "NH4",cobalt%f_nh4(i,j,k), "PO4", cobalt%f_po4(i,j,k),&
                    "FeLim", phyto(n)%def_fe(i,j,k), "EOpt", EOpt, "LOpt", LOpt, " p_2_n_opt ", phyto(n)%q_p_2_n_opt(i,j,k), &
                    " P Quota ",     ((cobalt%PE*EOpt+phyto(n)%PExtra + PStor*(EOpt+LOpt+S_plank))*(cobalt%molarP/cobalt%molarN)),  &  
                    " N Quota ", (S_plank*phyto(n)%NS+cobalt%NProt*(EOpt+LOpt)) 

                    call mpp_error(FATAL,"NaN in phytoplankton p_2_n_opt")
       ...
       enddo !} n
       cobalt%f_chl(i,j,k) =      cobalt%c_2_n*12.0e6*( phyto(SMALL)%f_n(i,j,k)*phyto(SMALL)%theta(i,j,k)+phyto(LARGE)%f_n(i,j,k)*phyto(LARGE)%theta(i,j,k)+ & 
               phyto(DIAZO)%f_n(i,j,k) * phyto(DIAZO)%theta(i,j,k))
   
    enddo;  enddo ; enddo !} i,j,k

        ! Switch back to original COBALT temporarily
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
       ...
       phyto(n)%jprod_nh4(i,j,k) = - min(0.0,phyto(n)%mu(i,j,k)*phyto(n)%f_n(i,j,k))
    ...
    do k = 1, nk  ;    do j = jsc, jec ;      do i = isc, iec   !{
       n=DIAZO
       phyto(n)%juptake_po4(i,j,k) = max( (phyto(n)%juptake_n2(i,j,k)+phyto(n)%juptake_nh4(i,j,k) + &
          phyto(n)%juptake_no3(i,j,k))*phyto(n)%q_p_2_n_opt(i,j,k), 0.0)
  ...
  cobalt%jprod_po4(i,j,k) = cobalt%jprod_po4(i,j,k) - &
          min(0.0,phyto(n)%mu(i,j,k)*phyto(n)%f_n(i,j,k))*phyto(n)%q_p_2_n(i,j,k)
       do n = 2, NUM_PHYTO
          phyto(n)%juptake_po4(i,j,k) = (phyto(n)%juptake_no3(i,j,k)+   &      !GIH
                  phyto(n)%juptake_nh4(i,j,k)) * phyto(n)%q_p_2_n_opt(i,j,k)          !GIH 
            phyto(n)%jprod_po4(i,j,k) =  - &
   min(0.0,phyto(n)%mu(i,j,k)*phyto(n)%f_n(i,j,k))*phyto(n)%q_p_2_n(i,j,k)
   
       cobalt%jprod_po4(i,j,k) = cobalt%jprod_po4(i,j,k) - &                !GIH 
                  min(0.0,phyto(n)%mu(i,j,k)*phyto(n)%f_n(i,j,k))*phyto(n)%q_p_2_n(i,j,k)
...
!
  ...
  !bact(1)%jprod_n(i,j,k) = min(bact(1)%jprod_n(i,j,k), &                            
  !                                  bact(1)%juptake_ldop(i,j,k)/bact(1)%q_p_2_n*bact(1)%gge_max- &
  !                                  bact(1)%f_n(i,j,k)/(cobalt%refuge_conc + bact(1)%f_n(i,j,k)) *      &
  !        bact(1)%temp_lim(i,j,k)*bact(1)%bresp*bact(1)%f_n(i,j,k))    ! GIH Changed the way P-lim handled
...
!
    ...
    ! 
    ! Set all static stoichiometric ratios outside k,j,i loop, except for the large and small plankton, which get set inside the
    ! loop since they are dynamic
    !
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec; !{
       ...
       prey_p2n_vec(3) = phyto(SMALL)%q_p_2_n(i,j,k)
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
         ...
         pre_totc(i,j,k) = (cobalt%p_dic(i,j,k,tau) + &
                    ...
                    cobalt%p_nsmz(i,j,k,tau) + cobalt%p_nmdz(i,j,k,tau) + &
                    cobalt%p_nlgz(i,j,k,tau)))*grid_tmask(i,j,k)
! GIH updating to reflect dynamic N:P
            pre_totp(i,j,k) = (cobalt%p_po4(i,j,k,tau) + cobalt%p_pdi(i,j,k,tau)  + &
                    cobalt%p_plg(i,j,k,tau)  + cobalt%p_psm(i,j,k,tau)  + &
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
       ...
       ! NAN CHECK
       if (ISNAN(cobalt%jndi(i,j,k))) then
               print *, " NAN in jndi ", " mu di ", phyto(DIAZO)%mu(i,j,k), " jzloss_n ",  phyto(DIAZO)%jzloss_n(i,j,k),       &
               " jhploss_n " ,   phyto(DIAZO)%jhploss_n(i,j,k), " jaggloss_n ", phyto(DIAZO)%jaggloss_n(i,j,k),       &
                " jvirloss_n ", phyto(DIAZO)%jvirloss_n(i,j,k), " jexuloss_n " , phyto(DIAZO)%jexuloss_n(i,j,k)
        end if
       !

       !
       ! Diazotrophic Phytoplankton Phosphorus (GIH)
       !
      
       cobalt%jpdi(i,j,k)=     (max(0.0,phyto(DIAZO)%mu(i,j,k))*phyto(DIAZO)%f_n(i,j,k)*phyto(DIAZO)%q_p_2_n_opt(i,j,k) + &
                            min(0.0, phyto(DIAZO)%mu(i,j,k))*phyto(DIAZO)%f_n(i,j,k)*phyto(DIAZO)%q_p_2_n(i,j,k) - &
        ...
        ! NAN CHECK
        if (ISNAN(cobalt%jpdi(i,j,k))) then
        print *, " NAN in jpdi ", " mu di " , phyto(DIAZO)%mu(i,j,k), " p_2_n opt ", phyto(DIAZO)%q_p_2_n_opt(i,j,k) , &
        " Diazo p_2_n ", phyto(DIAZO)%q_p_2_n(i,j,k),        " jzloss_p ", phyto(DIAZO)%jzloss_p(i,j,k) ,       &
        " jhploss_p ", phyto(DIAZO)%jhploss_p(i,j,k) , " jaggloss_p ", phyto(DIAZO)%jaggloss_p(i,j,k) ,       &
        " jvirloss_p ", phyto(DIAZO)%jvirloss_p(i,j,k) , " jexuloss_p ", phyto(DIAZO)%jexuloss_p(i,j,k), &
        " n diazo ", phyto(DIAZO)%f_n(i,j,k)


        end if
    

       cobalt%p_pdi(i,j,k,tau) = cobalt%p_pdi(i,j,k,tau) + dt*grid_tmask(i,j,k) * cobalt%jpdi(i,j,k) 
       ...
       ! NAN CHECK
       if (ISNAN(cobalt%jnlg(i,j,k))) then
               print *, " NAN in jnlg ", " mu lg ", phyto(LARGE)%mu(i,j,k), " jzloss_n ",  phyto(LARGE)%jzloss_n(i,j,k),       &
               " jhploss_n " ,   phyto(LARGE)%jhploss_n(i,j,k), " jaggloss_n ", phyto(LARGE)%jaggloss_n(i,j,k),       &
                " jvirloss_n ", phyto(LARGE)%jvirloss_n(i,j,k), " jexuloss_n " , phyto(LARGE)%jexuloss_n(i,j,k)
        end if
 

       ! 
       ! Large Phytoplankton Phosphorus (GIH) 
       ! (Note: I haven't bothered to declase cobalt%jplg terms because I don't see the jnlg terms used for anything in particular


        cobalt%jplg(i,j,k) =       ( max(phyto(LARGE)%mu(i,j,k),0.0)*phyto(LARGE)%f_n(i,j,k)*phyto(LARGE)%q_p_2_n_opt(i,j,k) &
               + min(phyto(LARGE)%mu(i,j,k),0.0)*phyto(LARGE)%f_n(i,j,k)*phyto(LARGE)%q_p_2_n(i,j,k)     &
               -  phyto(LARGE)%jzloss_p(i,j,k) - phyto(LARGE)%jhploss_p(i,j,k) -         &
                phyto(LARGE)%jaggloss_p(i,j,k) - phyto(LARGE)%jvirloss_p(i,j,k) - phyto(LARGE)%jexuloss_p(i,j,k) )


        ! NAN CHECK
        if (ISNAN(cobalt%jplg(i,j,k))) then
        print *, " NAN in jplg ", " mu lg " , phyto(LARGE)%mu(i,j,k), " p_2_n opt ", phyto(LARGE)%q_p_2_n_opt(i,j,k) , &
        " p_2_n actual ", phyto(LARGE)%q_p_2_n(i,j,k),        " jzloss_p ", phyto(LARGE)%jzloss_p(i,j,k) ,       &
        " jhploss_p ", phyto(LARGE)%jhploss_p(i,j,k) , " jaggloss_p ", phyto(LARGE)%jaggloss_p(i,j,k) ,       &
        " jvirloss_p ", phyto(LARGE)%jvirloss_p(i,j,k) , " jexuloss_p ", phyto(LARGE)%jexuloss_p(i,j,k)


        end if
 

       cobalt%p_plg(i,j,k,tau) = cobalt%p_plg(i,j,k,tau) + dt * grid_tmask(i,j,k) * cobalt%jplg(i,j,k)
       ...
       ! NAN CHECK
       if (ISNAN(cobalt%jnsm(i,j,k))) then
               print *, " NAN in jnsm ", " mu sm ", phyto(SMALL)%mu(i,j,k), " jzloss_n ",  phyto(SMALL)%jzloss_n(i,j,k),       &
               " jhploss_n " ,   phyto(SMALL)%jhploss_n(i,j,k), " jaggloss_n ", phyto(SMALL)%jaggloss_n(i,j,k),       &
                " jvirloss_n ", phyto(SMALL)%jvirloss_n(i,j,k), " jexuloss_n " , phyto(SMALL)%jexuloss_n(i,j,k)
        end if
       ...
       ! 
       ! Small Phytoplankton Phosphorus (GIH) 
       ! (Note: I haven't bothered to declase cobalt%jpsm terms because I don't see the jnlg terms used for anything in particular
        cobalt%jpsm(i,j,k) =  (max(phyto(SMALL)%mu(i,j,k),0.0)*phyto(SMALL)%f_n(i,j,k)*phyto(SMALL)%q_p_2_n_opt(i,j,k) & 
              + min(phyto(SMALL)%mu(i,j,k),0.0)*phyto(SMALL)%f_n(i,j,k)*phyto(SMALL)%q_p_2_n(i,j,k) -  phyto(SMALL)%jzloss_p(i,j,k) - phyto(SMALL)%jhploss_p(i,j,k) -         &
                phyto(SMALL)%jaggloss_p(i,j,k) - phyto(SMALL)%jvirloss_p(i,j,k) - phyto(SMALL)%jexuloss_p(i,j,k) )



        ! NAN CHECK
        if (ISNAN(cobalt%jpsm(i,j,k))) then
        print *, " NAN in jpsm ", " mu sm " , phyto(SMALL)%mu(i,j,k), " p_2_n opt ", phyto(SMALL)%q_p_2_n_opt(i,j,k) , &
         " p_2_n actual ", phyto(LARGE)%q_p_2_n(i,j,k),  " jzloss_p ", phyto(SMALL)%jzloss_p(i,j,k) ,       &
        " jhploss_p ", phyto(SMALL)%jhploss_p(i,j,k) , " jaggloss_p ", phyto(SMALL)%jaggloss_p(i,j,k) ,       &
        " jvirloss_p ", phyto(SMALL)%jvirloss_p(i,j,k) , " jexuloss_p ", phyto(SMALL)%jexuloss_p(i,j,k)

        end if
 

       cobalt%p_psm(i,j,k,tau) = cobalt%p_psm(i,j,k,tau) + dt * grid_tmask(i,j,k) * cobalt%jpsm(i,j,k)
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
       cobalt%jno3(i,j,k) =  cobalt%jnitrif(i,j,k) - phyto(DIAZO)%juptake_no3(i,j,k) -  &
           ...
           ! NaN Check
       if (ISNAN(cobalt%jno3(i,j,k))) then
            print *, " NAN in jno3 ", " jnitrif ", cobalt%jnitrif(i,j,k), " diazo juptake no3 ", &
            phyto(DIAZO)%juptake_no3(i,j,k), " large juptake no3 ", phyto(LARGE)%juptake_no3(i,j,k), &
            " small juptake no3 ", phyto(SMALL)%juptake_no3(i,j,k) , " jno3denitrif ", cobalt%jno3denit_wc(i,j,k)
       end if       
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
       ...
       ! NaN Check
       if (ISNAN(cobalt%jnh4(i,j,k))) then
            print *, " NAN in jnh4 ", " jprod_nh4 ", cobalt%jprod_nh4(i,j,k), " diazo juptake nh4 ", &
            phyto(DIAZO)%juptake_nh4(i,j,k), " large juptake nh4 ", phyto(LARGE)%juptake_nh4(i,j,k), &
            " small juptake nh4 ", phyto(SMALL)%juptake_nh4(i,j,k) , " jnitrif ", cobalt%jnitrif(i,j,k)
       end if       
        !
       ...
       cobalt%jpo4(i,j,k) = cobalt%jprod_po4(i,j,k) - phyto(DIAZO)%juptake_po4(i,j,k) - &
           ...
           ! NaN Check
       if (ISNAN(cobalt%jpo4(i,j,k))) then
            print *, " NAN in jpo4 ", " jprod_po4 ", cobalt%jprod_po4(i,j,k), " diazo juptake po4 ", &
            phyto(DIAZO)%juptake_po4(i,j,k), " large juptake po4 ", phyto(LARGE)%juptake_po4(i,j,k), &
            " small juptake po4 ", phyto(SMALL)%juptake_po4(i,j,k)
       end if       
...
!
    ...
    call g_tracer_set_values(tracer_list,'f_day_length' ,'field',cobalt%f_day_length ,isd,jsd,ntau=1)
    call g_tracer_set_values(tracer_list,'f_day_flag' ,'field',cobalt%f_day_flag ,isd,jsd,ntau=1)
     
    call g_tracer_set_values(tracer_list,'irrlim_mem_Di' ,'field',phyto(DIAZO)%irrlim_mem ,isd,jsd,ntau=1)
    call g_tracer_set_values(tracer_list,'irrlim_mem_Lg' ,'field',phyto(LARGE)%irrlim_mem ,isd,jsd,ntau=1)
    call g_tracer_set_values(tracer_list,'irrlim_mem_Sm' ,'field',phyto(SMALL)%irrlim_mem ,isd,jsd,ntau=1)
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
        ...
        imbal = (post_totc(i,j,k) - pre_totc(i,j,k))*86400.0/dt*1.03e6
         if ((abs(imbal).gt.1.0e-5) .OR. (ISNAN(imbal))) then
        print *, "C imbal   ", "imbal", imbal, " p_2_n small ", phyto(SMALL)%q_p_2_n(i,j,k) , &
                    " mu small ", phyto(SMALL)%mu(i,j,k), & 
                    "Temperature", Temp(i,j,k), "NO3", cobalt%f_no3(i,j,k), "NH4",cobalt%f_nh4(i,j,k), "PO4", cobalt%f_po4(i,j,k), &
                    " Irradiance ", cobalt%f_irr_mem(i,j,k)

        print *,  " p_2_n small ", phyto(SMALL)%q_p_2_n(i,j,k) , " mu small ", phyto(SMALL)%mu(i,j,k), & 
        " juptake no3 small ", phyto(SMALL)%juptake_no3(i,j,k), &
                   " juptake nh4 SMALL ", phyto(SMALL)%juptake_nh4(i,j,k), " juptake N small ",phyto(SMALL)%juptake_no3(i,j,k) + &
                    phyto(SMALL)%juptake_nh4(i,j,k), " juptake po4 small ", phyto(SMALL)%juptake_po4(i,j,k), &
                    " juptake P:N small", phyto(SMALL)%juptake_po4(i,j,k)/( phyto(SMALL)%juptake_no3(i,j,k) + &
                    phyto(SMALL)%juptake_nh4(i,j,k)), &
                    " jzloss P:N " , phyto(SMALL)%jzloss_p(i,j,k)/phyto(SMALL)%jzloss_n(i,j,k), " jvirloss P:N Small ", &
                    phyto(SMALL)%jvirloss_p(i,j,k)/phyto(SMALL)%jvirloss_n(i,j,k), " jexuloss P:N small ", &
                    phyto(SMALL)%jexuloss_p(i,j,k)/max(epsln, phyto(SMALL)%jexuloss_n(i,j,k)), &
                     " small jprod nh4 ", phyto(SMALL)%jprod_nh4(i,j,k), &
                    "small jprod po4 ", phyto(SMALL)%jprod_po4(i,j,k), " small jprod po4/nh4 ",& 
                    phyto(SMALL)%jprod_po4(i,j,k)/(epsln+phyto(SMALL)%jprod_nh4(i,j,k)), &
                    " small n flux ", cobalt%jnsm(i,j,k), " small p flux ", &
                    cobalt%jpsm(i,j,k)  , " small P:N flux ", cobalt%jpsm(i,j,k)/(epsln+cobalt%jnsm(i,j,k))



        print *,  " p_2_n large ", phyto(LARGE)%q_p_2_n(i,j,k) , " mu large ", phyto(LARGE)%mu(i,j,k), & 
        " juptake no3 large ", phyto(LARGE)%juptake_no3(i,j,k), &
                   " juptake nh4 LARGE ", phyto(LARGE)%juptake_nh4(i,j,k), " juptake N large ",phyto(LARGE)%juptake_no3(i,j,k) + &
                    phyto(LARGE)%juptake_nh4(i,j,k), " juptake po4 large ", phyto(LARGE)%juptake_po4(i,j,k), &
                    " juptake P:N large", phyto(LARGE)%juptake_po4(i,j,k)/( phyto(LARGE)%juptake_no3(i,j,k) + &
                    phyto(LARGE)%juptake_nh4(i,j,k)), &
                    " jzloss P:N " , phyto(LARGE)%jzloss_p(i,j,k)/phyto(LARGE)%jzloss_n(i,j,k), " jvirloss P:N Large ", &
                    phyto(LARGE)%jvirloss_p(i,j,k)/phyto(LARGE)%jvirloss_n(i,j,k), " jexuloss P:N large ", &
                    phyto(LARGE)%jexuloss_p(i,j,k)/max(epsln, phyto(LARGE)%jexuloss_n(i,j,k)), &
                     " large jprod nh4 ", phyto(LARGE)%jprod_nh4(i,j,k), &
                    "large jprod po4 ", phyto(LARGE)%jprod_po4(i,j,k), " large jprod po4/nh4 ", &
                    phyto(LARGE)%jprod_po4(i,j,k)/(epsln+phyto(LARGE)%jprod_nh4(i,j,k)), &
                    " large n flux ", cobalt%jnlg(i,j,k), " large p flux ", &
                    cobalt%jplg(i,j,k)  , " large P:N flux ", cobalt%jplg(i,j,k)/(epsln+cobalt%jnlg(i,j,k))


        print *,  " p_2_n diazo ", phyto(DIAZO)%q_p_2_n(i,j,k) , " mu diazo ", phyto(DIAZO)%mu(i,j,k), & 
        " juptake no3 diazo ", phyto(DIAZO)%juptake_no3(i,j,k), &
                   " juptake nh4 DIAZO ", phyto(DIAZO)%juptake_nh4(i,j,k), " juptake N diazo ",phyto(DIAZO)%juptake_no3(i,j,k) + &
                    phyto(DIAZO)%juptake_nh4(i,j,k), " juptake po4 diazo ", phyto(DIAZO)%juptake_po4(i,j,k), &
                    " juptake P:N diazo", phyto(DIAZO)%juptake_po4(i,j,k)/( phyto(DIAZO)%juptake_no3(i,j,k) + &
                    phyto(DIAZO)%juptake_nh4(i,j,k)), &
                    " jzloss P:N " , phyto(DIAZO)%jzloss_p(i,j,k)/phyto(DIAZO)%jzloss_n(i,j,k), " jvirloss P:N diazo ", &
                    phyto(DIAZO)%jvirloss_p(i,j,k)/phyto(DIAZO)%jvirloss_n(i,j,k), " jexuloss P:N diazo ", &
                    phyto(DIAZO)%jexuloss_p(i,j,k)/max(epsln, phyto(DIAZO)%jexuloss_n(i,j,k)), &
                    " diazo jprod nh4 ", phyto(DIAZO)%jprod_nh4(i,j,k), &
                    "diazo jprod po4 ", phyto(DIAZO)%jprod_po4(i,j,k), " diazo jprod po4/nh4 ",&
                    phyto(DIAZO)%jprod_po4(i,j,k)/(epsln+phyto(DIAZO)%jprod_nh4(i,j,k)), &
                    " diazo n flux ", cobalt%jndi(i,j,k), " diazo p flux ", &
                    cobalt%jpdi(i,j,k)  , " diazo P:N flux ", cobalt%jpdi(i,j,k)/(epsln+cobalt%jndi(i,j,k))
        ...
        ! GIH: Changing post_totp to reflect dynamic p_2_n
         ...
         if ((abs(imbal).gt.1.0e-5) .OR. (ISNAN(imbal))) then
          print *, "P imbal   ", "imbal", imbal, &
           "Temperature", Temp(i,j,k), "NO3", cobalt%f_no3(i,j,k),& 
          "NH4",cobalt%f_nh4(i,j,k), "PO4", cobalt%f_po4(i,j,k), &
                    " Irradiance ", cobalt%f_irr_mem(i,j,k)
        ...
        " juptake no3 diazo ", phyto(DIAZO)%juptake_no3(i,j,k), &
         ...
         if (abs(imbal).gt.1.0e-5) then   ! GIH Changed for testing purposes
           call mpp_error(FATAL,&
...
! GIH: Updating to reflect dynamic N:P
    ...
    cobalt%tot_layer_int_p(:,:,:) = (cobalt%p_po4(:,:,:,tau) + &
         cobalt%p_pdi(:,:,:,tau) + cobalt%p_plg(:,:,:,tau) + cobalt%p_psm(:,:,:,tau) + &
...
! GIH Added f_p_100
          ...
          phyto(n)%f_p_100(i,j) = phyto(n)%f_p(i,j,1) * rho_dzt(i,j,1)
...
! GIH: Added phyto(n)%f_p_100

                phyto(n)%f_p_100(i,j) = phyto(n)%f_p_100(i,j) + phyto(n)%f_p(i,j,k)*rho_dzt(i,j,k) 
...
! GIH: Added phyto(n)%f_p_100
              phyto(n)%f_p_100(i,j) = phyto(n)%f_p_100(i,j) + phyto(n)%f_p(i,j,k_100)*drho_dzt
! added juptake_fe_100 
...
! Send phytoplankton diagnostic data
    ...
    do n= 1, NUM_PHYTO
       ...
       if (phyto(n)%id_felim .gt. 0)           &
            ...
            model_time, rmask = grid_tmask,& 
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
       if (phyto(n)%id_irrlim_mem .gt. 0)          &
            used = g_send_data(phyto(n)%id_irrlim_mem,     phyto(n)%irrlim_mem,           &
            model_time, rmask = grid_tmask,& 
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    ...
    if (phyto(n)%id_irrlim .gt. 0)          &
       ...
       if (phyto(n)%id_liebig_lim .gt. 0)      &
        ...
        if (phyto(n)%id_EOpt .gt. 0)              &
            used = g_send_data(phyto(n)%id_EOpt,        phyto(n)%EOpt,                  &
            model_time, rmask = grid_tmask,&       
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
            
    if (phyto(n)%id_LOpt .gt. 0)              &
            used = g_send_data(phyto(n)%id_LOpt,        phyto(n)%LOpt,                  &
            model_time, rmask = grid_tmask,& 
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)

          
    if (phyto(n)%id_LDiag .gt. 0)              &
            used = g_send_data(phyto(n)%id_LDiag,        phyto(n)%LDiag,                  &
            model_time, rmask = grid_tmask,& 
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
...
! GIH Added phyto(n)%id_q_p_2_n
       if (phyto(n)%id_q_p_2_n_opt .gt. 0)        &
            used = g_send_data(phyto(n)%id_q_p_2_n_opt,   phyto(n)%q_p_2_n_opt,           &
            model_time, rmask = grid_tmask,& 
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
 ...
 if (cobalt%id_expkPhotoT .gt. 0)              &
         used = g_send_data(cobalt%id_expkPhotoT,       cobalt%expkPhotoT,       &
         model_time, rmask = grid_tmask,&
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
   ...
   if (cobalt%id_jo2resp_wc .gt. 0)            &
    ...
    if (cobalt%id_nphyto_tot .gt. 0)                &
         ...
         model_time, rmask = grid_tmask(:,:,:),& 
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
! GIH Add cobalt%id_pphyto_tot
    if (cobalt%id_pphyto_tot .gt. 0)                &
         used = g_send_data(cobalt%id_pphyto_tot,   (cobalt%p_pdi(:,:,:,tau) +  &
         cobalt%p_plg(:,:,:,tau) + cobalt%p_psm(:,:,:,tau)), &
         model_time, rmask = grid_tmask(:,:,:),& 
         is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
...
!
   ...
   if (cobalt%id_sfc_irr .gt. 0)              &
    ...
    if (cobalt%id_sfc_irr_mem .gt. 0)          &
         ...
         model_time, rmask = grid_tmask(:,:,1),&
         is_in=isc, js_in=jsc,ie_in=iec, je_in=jec)
    if (cobalt%id_sfc_day_length .gt. 0)          &
         used = g_send_data(cobalt%id_sfc_day_length,  cobalt%f_day_length(:,:,1),       &
         model_time, rmask = grid_tmask(:,:,1),&
         is_in=isc, js_in=jsc,ie_in=iec, je_in=jec)
   ...
   if (cobalt%id_btm_co3_sol_calc .gt. 0)            &
    ...
    do n= 1, NUM_PHYTO
       if (phyto(n)%id_sfc_f_n .gt. 0)              &
          ...
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
! GIH Added phyto(n)%id_sfc_f_p
        if (phyto(n)%id_sfc_f_p .gt. 0)              &
          used = g_send_data(phyto(n)%id_sfc_f_p, phyto(n)%f_p(:,:,1),            & 
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
       ...
       if (phyto(n)%id_sfc_felim .gt. 0)              &
          ...
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      if (phyto(n)%id_sfc_irrlim_mem .gt. 0)              &
          used = g_send_data(phyto(n)%id_sfc_irrlim_mem, phyto(n)%irrlim_mem(:,:,1),      &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  ...
  if (phyto(n)%id_sfc_irrlim .gt. 0)              &
     ...
     if (phyto(n)%id_sfc_mu .gt. 0)              &
      ...
      if (phyto(n)%id_sfc_EOpt .gt. 0)              &
          used = g_send_data(phyto(n)%id_sfc_EOpt, phyto(n)%EOpt(:,:,1),      &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  
      if (phyto(n)%id_sfc_LOpt .gt. 0)              &
          used = g_send_data(phyto(n)%id_sfc_LOpt, phyto(n)%LOpt(:,:,1),      &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  
   
      if (phyto(n)%id_sfc_LDiag .gt. 0)              &
          used = g_send_data(phyto(n)%id_sfc_LDiag, phyto(n)%LDiag(:,:,1),      &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  

      if (phyto(n)%id_sfc_liebig_lim .gt. 0)              &
          used = g_send_data(phyto(n)%id_sfc_liebig_lim, phyto(n)%liebig_lim(:,:,1),      &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  ...
  if (phyto(n)%id_sfc_q_p_2_n_opt .gt. 0)              &
          used = g_send_data(phyto(n)%id_sfc_q_p_2_n_opt, phyto(n)%q_p_2_n_opt(:,:,1),      &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  ...
  if (phyto(n)%id_sfc_q_fe_2_n .gt. 0)              &
    ...
    do n= 1, NUM_PHYTO  !{
      ...
      if (phyto(n)%id_f_n_100 .gt. 0)     &
          ...
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
! GIH: Added this line for id_f_p_100
      if (phyto(n)%id_f_p_100 .gt. 0)     &
          used = g_send_data(phyto(n)%id_f_p_100, phyto(n)%f_p_100,         &
          model_time, rmask = grid_tmask(:,:,1),&
          is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
...
! GIH: Changed this to reflect dynamics p_2_n ratios
...
! GIH: Updated to reflect new dynamics p_2_n
...
! GIH: Changed this to reflect dynamic stoichiometry and then became confused, replaced a dynamics diazotrophy term but the rest of
! the terms had N units- don't understand
    if (cobalt%id_popos .gt. 0)            &
        used = g_send_data(cobalt%id_popos,  (( cobalt%p_pdi(:,:,1,tau)) + cobalt%p_plg(:,:,1,tau) + &
        cobalt%p_psm(:,:,1,tau) + cobalt%p_nbact(:,:,1,tau)*bact(1)%q_p_2_n +  cobalt%p_pdet(:,:,1,tau) + &
        cobalt%p_nsmz(:,:,1,tau)*zoo(1)%q_p_2_n + cobalt%p_nmdz(:,:,1,tau)*zoo(2)%q_p_2_n + &
        cobalt%p_nlgz(:,:,1,tau)*zoo(3)%q_p_2_n) * cobalt%Rho_0,  &
        model_time, rmask = grid_tmask(:,:,1),&
        is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
...
! GIH Added phyto(n)%f_p
       ...
       allocate(phyto(n)%irrlim_mem(isd:ied,jsd:jed,nk))   ; phyto(n)%irrlim_mem     = 0.0
       ...
       allocate(phyto(n)%LOpt(isd:ied,jsd:jed,nk))           ; phyto(n)%LOpt             = 0.0
   
       allocate(phyto(n)%LDiag(isd:ied,jsd:jed,nk))           ; phyto(n)%LDiag             = 0.0
       allocate(phyto(n)%EOpt(isd:ied,jsd:jed,nk))           ; phyto(n)%EOpt             = 0.0
...
! GIH Added phyto(n)%q_p_2_n and q_p_2_opt
...
!    allocate(bact(1)%q_p_2_n(isd:ied,jsd:jed,nk))            ; bact(1)%q_p_2_n           = 0.0
    ...
    allocate(cobalt%expkPhotoT(isd:ied, jsd:jed, 1:nk))        ; cobalt%expkPhotoT=0.0
...
!==============================================================================================================
    !
    ! allocate day length quantities

    !

    allocate(cobalt%f_day_length(isd:ied, jsd:jed, 1:nk))         ; cobalt%f_day_length=0.0
    allocate(cobalt%f_day_flag(isd:ied, jsd:jed, 1:nk))         ; cobalt%f_day_flag=0.0
...
!GIH Added phyto(n)%f_p_100
       allocate(phyto(n)%f_p_100(isd:ied,jsd:jed))          ; phyto(n)%f_p_100  = 0.0
  ...
  subroutine user_deallocate_arrays
    ...
    do n = 1, NUM_PHYTO
       ...
       deallocate(phyto(n)%LDiag)
       deallocate(phyto(n)%LOpt)
       deallocate(phyto(n)%EOpt)
       deallocate(phyto(n)%mu)
       deallocate(phyto(n)%po4lim)
       deallocate(phyto(n)%q_fe_2_n)

       ! GIH Added phyto(n)%q_p_2_n and opt
...
!==============================================================================================================
! Deallocate Day Length Quantities


    deallocate(cobalt%f_day_length) 

    deallocate(cobalt%f_day_flag)  
!==============================================================================================================
...
! GIH Added deallocate phyto(n)%f_p_100
       deallocate(phyto(n)%f_p_100)
    enddo
